...
export default function RatingFilter({ queryConfig }: { queryConfig: QueryConfig }) {
  const navigate = useNavigate()
  const handleFilterStart = (index: number) => {
    return navigate({
      pathname: path.home,
      search: createSearchParams({ ...queryConfig, rating_filter: String(5 - index) }).toString()
    })
  }
  return (
    <div>
        ...
        {Array(5)
          ...
          .map((_, index) => (
            <li className='' key={index}>
              <div className='flex items-center my-2 cursor-pointer' onClick={() => handleFilterStart(index)}>
                {Array(5)
                  .fill(0)
                  .map((_, indexFilter) => {
                    if (indexFilter < 5 - index) {
                      return (
                        <svg key={indexFilter} viewBox='0 0 9.5 8' className='w-4 h-4'>
                          <defs>
                    ...
                    return (
                      <svg key={indexFilter} viewBox='0 0 30 30' className='w-4 h-4'>
                        <defs>
                ...
                {index !== 0 && <span className='px-1 text-sm'>Trở lên</span>}
              </div>
            </li>
          ))}
      </ul>
