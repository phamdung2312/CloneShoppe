...
export default function Input<TFieldValues extends FieldValues>({
  ...
  const registerValidate = name && register ? register(name, rules) : {}
  const toggleEye = () => {
  ...
  return (
    <div className={className}>
      <input className={classNameInput} {...registerValidate} {...rest} type={handleTypePassword()} />
      {rest.type === 'password' && openEye && (
        <svg
          ...
          strokeWidth={1.5}
          stroke='currentColor'
          className={`size-6 absolute ${classNameEyePassword} stroke-black cursor-pointer`}
        >
          <path
            strokeLinecap='round'
